...
function site({ pathname = '/', storage = new Map(), errorPage = false, portraits = false } = {}) {
    ...
    const cards = portraits ? Array.from({ length: 8 }, (_, i) => {
...
test('nosy requires all eight distinct Hall of Fame portraits and persists between visits', () => {
    let page = site({ portraits: true });
    for (let i = 0; i < 8; i++) page.cards[0].emit('click');
    assert.equal(page.awards().nosy, undefined);
    for (let i = 1; i < 7; i++) page.cards[i].emit('click');
    assert.equal(page.awards().nosy, undefined);
    page = site({ storage: page.storage, portraits: true });
    page.cards[7].emit('keydown', { key: 'Enter', repeat: true });
    assert.equal(page.awards().nosy, undefined);
    page.cards[7].emit('keydown', { key: 'Enter' });
