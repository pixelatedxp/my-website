(function () {
    ...
    function init() {
        ...
        css.textContent =
            '.music-player{position:static;display:grid;grid-template-columns:auto minmax(0,165px) auto;align-items:center;gap:10px;width:max-content;max-width:calc(100vw - 40px);margin:24px 20px 20px auto;padding:10px 12px;background:var(--card-bg,rgba(30,30,35,.88));border:1px solid var(--card-border,rgba(255,255,255,.08));border-radius:16px;backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);box-shadow:0 8px 32px rgba(0,0,0,.28);opacity:0;transform:translateY(16px);transition:opacity .7s cubic-bezier(.16,1,.3,1),transform .7s cubic-bezier(.16,1,.3,1),background-color .4s,border-color .4s;pointer-events:none}' +
            ...
            '@media(max-width:600px){.music-player{grid-template-columns:auto minmax(0,125px) auto;max-width:calc(100vw - 24px);margin:18px 12px 16px auto;gap:7px}.music-thumb{width:40px;height:40px}.music-btn{padding:3px}}@media(prefers-reduced-motion:reduce){.music-track-inner.scrolling{animation:none}}';
